...
public class ScavengingHyenaDE extends DeathEffect {
	...
	public MyTurnState perform(MyTurnState oldstate, Minion source, Minion minion) {
		if (minion.getRace().equals(Race.BEAST) && ((source.getMyPos()<7 && minion.getMyPos()<7) || (source.getMyPos()>=7 && minion.getMyPos()>=7))) return oldstate.applyBuff(source.getId(), new AdditiveBuff(-1,2,1,0));
